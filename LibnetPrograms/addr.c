...
int main()
{
  ...
  if (ip_addr != -1)
  {
    ...
    /* libnet_name2addr4 returns the address in network
     * order (big endian). */
    ...
    */
    ...
    // cast &ip_addr into a u_int8_t* and store it in ip_addr_p. We are turning what would be a pointer to a 4 byte integer into a pointer to an array of 4 single bytes.
    printf("Address read: %d.%d.%d.%d\n", ip_addr_p[0], ip_addr_p[1], ip_addr_p[2], ip_addr_p[3]);

    /* This would output the same thing, but I wanted to
